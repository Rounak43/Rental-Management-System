...
const AddProduct = () => {
  ...
  // State to hold preview list: [{ file: File, url: string, isCover: boolean }]
  ...
  const handleFileChange = (e) => {
    ...
    if (imagePreviews.length + files.length > 1) {
    ...
    // If no cover image exists, mark the first preview as cover
    const hasCover = imagePreviews.some(p => p.isCover);
    if (!hasCover && newPreviews.length > 0) {
  ...
  const handleRemoveImage = (index) => {
    ...
    // If we removed the cover, promote the new first image as cover
  ...
  const handleSetCover = (index) => {
    const updated = imagePreviews.map((p, i) => ({
      ...p,
      isCover: i === index
    }));
    setImagePreviews(updated);
  ...
  const handleMoveImage = (index, direction) => {
    ...
    if (direction === 'right' && index === imagePreviews.length - 1) return;
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // 1. Upload files first — wrap in FormData with the 'images' field Multer expects
      const filesToUpload = imagePreviews.map(p => p.file);
      const uploadForm = new FormData();
      filesToUpload.forEach(file => uploadForm.append('images', file));
      const uploadRes = await uploadProductImages(uploadForm);
      const uploadedPaths = uploadRes?.filePaths;
      
      if (!uploadedPaths || uploadedPaths.length === 0) {
        throw new Error('Image uploading failed.');
      }

      // Reorder backend paths based on cover status
      const coverIndex = imagePreviews.findIndex(p => p.isCover);
      const reorderedPaths = [...uploadedPaths];
      
      if (coverIndex > 0) {
        // Move cover path to first index
        const coverPath = reorderedPaths[coverIndex];
        reorderedPaths.splice(coverIndex, 1);
        reorderedPaths.unshift(coverPath);
      }

      // 2. Submit Product payload
      const payload = {
        ...formData,
        ...
        images: reorderedPaths,
        image: reorderedPaths[0] // Set cover path to legacy single image field
      };
        ...
        <main style={{ flex: 1, padding: '32px', background: 'var(--bg-color)', overflowY: 'auto' }}>
          <div className="partner-dash-container" style={{ maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <h2 style={{ margin: 0 }}>Add Rental Equipment</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="glass-card" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div className="form-group" style={{ gridColumn: 'span 2' }}>
                  ...
                  <textarea 
                    name="description" 
                    rows="5"
                    placeholder="Enter specs, inclusions (chargers, batteries, stands) and usage guidelines..."
              ...
              <div className="form-group" style={{ borderTop: '1px solid var(--surface-border)', paddingTop: '20px' }}>
                <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{imagePreviews.length}/1 uploaded</span>
                </label>
                ...
                {imagePreviews.length < 1 && (
                  <div style={{ border: '2px dashed var(--surface-border)', borderRadius: '12px', padding: '32px', textAlign: 'center', cursor: 'pointer', background: 'rgba(255, 102, 0, 0.02)', position: 'relative' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>JPG, PNG formats supported up to 5MB</span>
                  </div>
                )}
                ...
                {imagePreviews.length > 0 && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '16px', marginTop: '16px' }}>
                    {imagePreviews.map((preview, index) => (
                      <div key={index} className="glass-card" style={{ position: 'relative', height: '130px', borderRadius: '12px', overflow: 'hidden', border: preview.isCover ? '2px solid var(--primary-color)' : '1px solid var(--surface-border)' }}>
                        <img 
                          src={preview.url} 
                          alt="preview" 
                        ...
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '4px 0', zIndex: 10 }}>
                          ...
                          <button 
                            ...
                            style={{ background: 'none', border: 'none', color: preview.isCover ? 'var(--primary-color)' : 'white', cursor: 'pointer', padding: '2px', display: 'flex', alignItems: 'center' }}
                            title="Set Cover"
                          >
                            <Check size={14} />
                          </button>
                          ...
                          <button 
                            ...
                            style={{ background: 'none', border: 'none', color: index === imagePreviews.length - 1 ? 'rgba(255,255,255,0.3)' : 'white', cursor: 'pointer', padding: '2px' }}
                          >
